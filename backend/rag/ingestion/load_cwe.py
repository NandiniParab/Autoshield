...
def get_default_cwes() -> List[Dict]:
    ...
    return [
        ...
        {"id": "CWE-798", "name": "Use of Hard-coded Credentials", "owasp": "A07",
         "description": "The product contains hard-coded credentials, such as passwords, API keys, tokens, JWT secrets, or cryptographic keys. Hard-coded credentials can be recovered from source code, repositories, client bundles, logs, backups, or deployed artifacts and used to bypass authentication or access protected systems. Store secrets outside source code in a secret manager or environment-specific configuration and rotate exposed credentials."},
        {"id": "CWE-1104", "name": "Use of Unmaintained Third Party Components", "owasp": "A06",
         "description": "The product relies on third party components, packages, libraries, or dependencies that are no longer maintained, outdated, vulnerable, or unsupported. Vulnerable dependencies can expose applications to known CVEs such as prototype pollution, SSRF, denial of service, remote code execution, or authentication bypass. Keep dependency inventories current, monitor advisories, upgrade to fixed versions, remove unused packages, and use dependency scanning tools such as npm audit or pip-audit."},
