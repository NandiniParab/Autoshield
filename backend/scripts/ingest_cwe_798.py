...
def main() -> None:
    records = [
        ...
        if record.get("metadata", {}).get("cwe_id") == "CWE-798"
    ]
    if not records:
        raise RuntimeError("CWE-798 record was not found in load_cwe().")

    collection = get_collection()
    try:
        collection.delete(where={"cwe_id": "CWE-798"})
    except Exception as exc:
        print(f"[CWE-798] Existing delete skipped: {exc}")

    chunks = chunk_records(records)
    for index, chunk in enumerate(chunks):
        chunk["id"] = f"cwe-CWE-798-{index}"
    ...
    print(f"[CWE-798] Stored {count} chunk(s). Collection count: {stats['count']}")
